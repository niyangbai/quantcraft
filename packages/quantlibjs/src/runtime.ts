...
export interface AsianOptionInput extends EuropeanOptionInput { averageSoFar: number; pastFixings: number; futureFixings: number; }
export interface WorstOfOptionInput { evaluationDate: string; maturityDate: string; spot1: number; spot2: number; riskFreeRate: number; dividendYield1?: number; dividendYield2?: number; volatility1: number; volatility2: number; correlation: number; strike: number; type: "call" | "put"; }
export interface AutocallInput { evaluationDate: string; maturityDate: string; spot: number; initialSpot: number; riskFreeRate: number; dividendYield?: number; volatility: number; coupon: number; callLevel: number; barrierLevel: number; notional: number; observationMonths: number; paths: number; }
...
export class QuantLibRuntime {
    ...
    priceAsian(i: AsianOptionInput): PriceResult { return checked(this.ql.priceAsian(...ymd(i.evaluationDate), ...ymd(i.maturityDate), i.spot, i.strike, i.riskFreeRate, i.dividendYield ?? 0, i.volatility, i.type === "call", i.averageSoFar, i.pastFixings, i.futureFixings)); }
    priceWorstOf(i: WorstOfOptionInput): PriceResult { return checked(this.ql.priceWorstOf(...ymd(i.evaluationDate), ...ymd(i.maturityDate), i.spot1, i.spot2, i.riskFreeRate, i.dividendYield1 ?? 0, i.dividendYield2 ?? 0, i.volatility1, i.volatility2, i.correlation, i.strike, i.type === "call")); }
    priceAutocall(i: AutocallInput): PriceResult { return checked(this.ql.priceAutocall(...ymd(i.evaluationDate), ...ymd(i.maturityDate), i.spot, i.initialSpot, i.riskFreeRate, i.dividendYield ?? 0, i.volatility, i.coupon, i.callLevel, i.barrierLevel, i.notional, i.observationMonths, i.paths)); }
