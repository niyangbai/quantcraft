...
export function QuestionBankPanel({ bank, onInstallBank }: { bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void }) {
  ...
  const uploadBank = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBankMessage({ ok: true, text: `Loaded ${parsed.payoff.length} Payoff position seeds, ${parsed.greek.scenarios.length} scenarios, ${parsed.greek.books.length} Greek books, and ${parsed.hedge.products.length} Hedge products.` });
    } catch (error) {
...
export function Landing({ scoreboard, bank, onInstallBank, onDifficulty, onSelect }: { scoreboard: Scoreboard; bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void; onDifficulty: (difficulty: Difficulty) => void; onSelect: (mode: Mode) => void }) {
  const total = scoreboard.payoff.score + scoreboard.greek.score + scoreboard.hedge.score;
