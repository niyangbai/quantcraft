...
export function QuestionBankPanel({ bank, onInstallBank }: { bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void }) {
  ...
  const uploadBank = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBankMessage({ ok: true, text: `Loaded ${parsed.payoff.length} Payoff position seeds, ${parsed.greekthon.scenarios.length} scenarios, ${parsed.greekthon.books.length} Greekthon books, and ${parsed.hedge.products.length} Hedge products.` });
    } catch (error) {
...
export function Collection({ name, scoreboard, onRename, onResetScore, onBack }: { name: string; scoreboard: Scoreboard; onRename: (name: string) => void; onResetScore: () => void; onBack: () => void }) {
  ...
  const greekthonAccuracy = scoreboard.greekthon.answers ? scoreboard.greekthon.correct / scoreboard.greekthon.answers * 100 : 0;
  ...
  return (
    <section className="mode-view">
      ...
      <div className="settlement-grid">
        ...
        <article><span>GREEKTHON</span><strong>{scoreboard.greekthon.score}</strong><small>{scoreboard.greekthon.correct}/{scoreboard.greekthon.answers} correct · {greekthonAccuracy.toFixed(0)}% accuracy</small><b>BEST STREAK ×{scoreboard.greekthon.bestStreak}</b></article>
