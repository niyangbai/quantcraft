...
export function QuestionBankPanel({ bank, onInstallBank }: { bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void }) {
  ...
  const uploadBank = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBankMessage({ ok: true, text: `Loaded ${parsed.payoff.length} Payoff position seeds, ${parsed.greek.scenarios.length} scenarios, ${parsed.orderbook.length} Order Book templates, ${parsed.hedge.products.length} Hedge products, Make Market, Volatility, Curve, and Exotic model parameters.` });
    } catch (error) {
  ...
  return <section className="question-bank-panel"><div><p className="panel-label">QUESTION BANK</p><h2>Custom questions</h2><p>Upload one validated JSON file for all six games.</p><div className="bank-counts"><span><strong>{bank.payoff.length}</strong> Payoff books</span><span><strong>{bank.greek.scenarios.length}</strong> Scenarios</span><span><strong>{bank.greek.books.length}</strong> Greek books</span><span><strong>{bank.greek.metrics.length}</strong> KPIs</span><span><strong>{bank.orderbook.length}</strong> Order Book templates</span><span><strong>{bank.hedge.products.length}</strong> Hedge products</span><span><strong>{Object.keys(bank.makemarket).length}</strong> Make Market model params</span><span><strong>{Object.keys(bank.volatility).length}</strong> Volatility model params</span><span><strong>{Object.keys(bank.curve).length}</strong> Curve params</span><span><strong>{Object.keys(bank.exotic).length}</strong> Exotic params</span></div></div><div className="bank-actions"><button onClick={downloadExample}>DOWNLOAD EXAMPLE JSON</button><label>UPLOAD QUESTION BANK<input type="file" accept="application/json,.json" onChange={uploadBank} /></label></div>{bankMessage && <div className={bankMessage.ok ? "bank-message ok" : "bank-message error"}>{bankMessage.text}</div>}</section>;
}
...
export function Landing({ scoreboard, bank, onInstallBank, onDifficulty, onSelect }: { scoreboard: Scoreboard; bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void; onDifficulty: (difficulty: Difficulty) => void; onSelect: (mode: Mode) => void }) {
  return <section className="landing"><div className="landing-hero"><h1>Read the market.<br />Make the call.</h1><p>"Risk means more things can happen than will happen." — Elroy Dimson</p><div className="landing-run"><span>{scoreboard.difficulty === "intern" ? "INFINITE LIFE" : <>{"♥".repeat(scoreboard.lives)}{"♡".repeat(scoreboard.maxLives - scoreboard.lives)}</>}</span><strong>{totalScore(scoreboard)} PTS</strong><small>{scoreboard.difficulty.toUpperCase()} · {scoreboard.gameOver ? "RUN ENDED" : "RUN ACTIVE"}</small></div></div><div className="difficulty-picker"><div><p className="panel-label">DIFFICULTY</p><strong>Lives per run</strong></div>{([['intern','INTERN','Infinite life'],['vp','VP','5 lives'],['md','MD','1 life']] as [Difficulty,string,string][]).map(([id,label,detail]) => <button key={id} className={scoreboard.difficulty === id ? "active" : ""} onClick={() => onDifficulty(id)}><strong>{label}</strong><small>{detail}</small></button>)}</div><div className="mode-cards"><button onClick={() => onSelect("payoff")}><span>∑</span><strong>PAYOFF</strong><p>Call the terminal payoff, max profit, and breakeven.</p></button><button onClick={() => onSelect("greek")}><span>Δ</span><strong>GREEK</strong><p>Call the direction of fair value and Greeks.</p></button><button onClick={() => onSelect("orderbook")}><span>☰</span><strong>ORDER BOOK</strong><p>Read how a market order moves the ladder.</p></button><button onClick={() => onSelect("hedge")}><span>≋</span><strong>HEDGE</strong><p>Trade the book inside its risk limits.</p></button><button onClick={() => onSelect("makemarket")}><span>⇄</span><strong>MAKE MARKET</strong><p>Pick the quote the synthetic model scores best.</p></button><button onClick={() => onSelect("volatility")}><span>σ</span><strong>VOLATILITY</strong><p>Find the position the surface shock pays.</p></button><button onClick={() => onSelect("curve")}><span>∿</span><strong>CURVE</strong><p>Read the yield-curve move and find the biggest P&L.</p></button><button onClick={() => onSelect("exotic")}><span>◈</span><strong>EXOTIC</strong><p>Find the state that matters and the position that loses most.</p></button></div><QuestionBankPanel bank={bank} onInstallBank={onInstallBank} /></section>;
}
    ...
    <section className="mode-view">
      ...
      <div className="collection-heading">
        ...
        <div className="level-card">
          ...
          <strong>{totalScore(scoreboard)}</strong>
