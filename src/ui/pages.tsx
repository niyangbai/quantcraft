...
export function QuestionBankPanel({ bank, onInstallBank }: { bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void }) {
  ...
  const uploadBank = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBankMessage({ ok: true, text: `Loaded ${parsed.payoff.length} Payoff position seeds, ${parsed.greek.scenarios.length} scenarios, ${parsed.orderbook.length} Order Book templates, and ${parsed.hedge.products.length} Hedge products.` });
    } catch (error) {
  ...
  return <section className="question-bank-panel"><div><p className="panel-label">QUESTION BANK</p><h2>Custom questions</h2><p>Upload one validated JSON file for all four games.</p><div className="bank-counts"><span><strong>{bank.payoff.length}</strong> Payoff books</span><span><strong>{bank.greek.scenarios.length}</strong> Scenarios</span><span><strong>{bank.greek.books.length}</strong> Greek books</span><span><strong>{bank.greek.metrics.length}</strong> KPIs</span><span><strong>{bank.orderbook.length}</strong> Order Book templates</span><span><strong>{bank.hedge.products.length}</strong> Hedge products</span></div></div><div className="bank-actions"><button onClick={downloadExample}>DOWNLOAD EXAMPLE JSON</button><label>UPLOAD QUESTION BANK<input type="file" accept="application/json,.json" onChange={uploadBank} /></label></div>{bankMessage && <div className={bankMessage.ok ? "bank-message ok" : "bank-message error"}>{bankMessage.text}</div>}</section>;
}
