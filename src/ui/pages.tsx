...
export function QuestionBankPanel({ bank, onInstallBank }: { bank: QuestionBank; onInstallBank: (bank: QuestionBank) => void }) {
  ...
  const uploadBank = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBankMessage({ ok: true, text: `Loaded ${parsed.payoff.length} Payoff position seeds, ${parsed.greek.scenarios.length} scenarios, ${parsed.orderbook.length} Order Book templates, ${parsed.hedge.products.length} Hedge products, Make Market model parameters, and Volatility model parameters.` });
    } catch (error) {
